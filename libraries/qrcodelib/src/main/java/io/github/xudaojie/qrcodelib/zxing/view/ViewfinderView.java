...
//            paint.setColor(frameColor);
    ...
    private void drawText(Canvas canvas, Rect frame) {
        ...
        if (cameraPermission == PackageManager.PERMISSION_GRANTED) {
            ...
            String text = hint;
            canvas.drawText(hint, frame.centerX() - text.length() * textsize / 2, frame.bottom + textsize + 20, paint);
        } else {
            paint.setColor(errorHintColor);
            paint.setTextSize(textsize);
            String text = errorHint;
            canvas.drawText(errorHint, frame.centerX() - text.length() * textsize / 2, frame.bottom + textsize + 20, paint);
        }
    }
    ...
    private void drawScanner(Canvas canvas, Rect frame) {
        if (showPossiblePoint) {
            ...
            canvas.drawRect(frame.left + 10, frame.top, frame.right - 10, frame.top + 4, paint);

            translateY += 5f;
            if (translateY >= frame.bottom - frame.top) {
                translateY = 5f;
            }
        }
    }
